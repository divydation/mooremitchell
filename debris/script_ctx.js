...
window.addEventListener('resize', () => {
    // 1. Run your existing code that sets the canvas style width/height
    updateMyLayout(); 

    // 2. Tell PixiJS to resize its internal coordinate system
...
function mainThread() {
    ...
    for (let i = 0; i < planets.length; i++) {
        ...
        // ---------------------------------------------------
        // PHASE 1: CACHE COLLECTOR MATH (Runs for ALL planets)
        // ---------------------------------------------------
        ...
        // ---------------------------------------------------
        // PHASE 2: MATERIALS LOOP (Runs for ALL planets)
        // ---------------------------------------------------
        for (let i = 0; i < planet.materials.count; i++) {
            
            // 1. Basic Background Math (Always runs)
            planet.materials.radius[i] += planet.materials.radiusChange[i];
            planet.materials.value[i] *= 1.005;
            planet.materials.value[i] = Math.min(planet.materials.value[i], 1000); 

            let mRadius = planet.materials.radius[i];
            let mAngle = planet.materials.angle[i];
            
            // Cache Cartesian for collisions
            let materialPosition = polarToCartesian(mRadius, mAngle);
            planet.materials.x[i] = materialPosition.x;
            planet.materials.y[i] = materialPosition.y;
            ...
            if (drawThisPlanet) {
                ...
                if (distanceSq <= 225) {
                    material += Math.floor(planet.materials.value[i]);
                    deleteMaterial(planet, i); 
                    i--; 
                    continue; 
                } 

                // Ship Tractor Beam Pull (Visual Math)
                if (distanceSq <= collectionRadius**2) {
                    planet.materials.timeInTractorBeam[i] += 0.05;
                    let beamTime = Math.min(planet.materials.timeInTractorBeam[i], 1);
                    
                    planet.materials.radius[i] += (flightRadius + 7.5 - mRadius) * beamTime;
                    
                    let angleDiff = Math.atan2(Math.sin(shipRotation - mAngle), Math.cos(shipRotation - mAngle));
                    planet.materials.angle[i] += (angleDiff * beamTime) + toRadians(0.5);
                } 
            }
            ...
            for (let c = 0; c < planet.collectors.length; c++) {
                ...
                let dx = planet.materials.x[i] - collector.x;
                let dy = planet.materials.y[i] - collector.y;
                let distanceSq = (dx * dx) + (dy * dy); // Pure, fast math
                ...
                if (drawThisPlanet && distanceSq <= collectionRadius**2) {
                    planet.materials.timeInTractorBeam[i] += 0.05;
                    let beamTime = Math.min(planet.materials.timeInTractorBeam[i], 1);
                    planet.materials.radius[i] += (collector.radius + 5 - mRadius) * beamTime;
            ...
            // 4. CLEANUP & RENDER
            // -----------------------------------------
            if (planet.materials.radius[i] > 600 || planet.materials.radius[i] < 10) {
                planet.materials.alpha[i] -= 0.1;
            }

            if (planet.materials.alpha[i] < 0) {
                deleteMaterial(planet, i);
                i--;
                continue;
            }

            if (drawThisPlanet) canvasDrawMaterials(i, planet.materials);
        }

        // ---------------------------------------------------
        // PHASE 3: DRAW COLLECTORS LAST 
        // ---------------------------------------------------
        for (let i = 0; i < planet.collectors.length; i++) {
            let p = planet.collectors[i];
            if (drawThisPlanet) canvasDrawCollector(p); 
        }
...
function deleteMaterial(planet, index) {
    ...
    // If the rock we are deleting isn't already the very last rock,
    // copy the last rock's data into this slot to fill the hole.
    if (index !== last) {
    ...
    // Shrink the active pool by 1. The old 'last' data still exists in 
    // memory, but because count is smaller, the game completely ignores it.
    planet.materials.count--;
}
...
function canvasDrawMaterials(index, mats) {
    ctx.save();
    
    // Read the cached X and Y we saved in the physics loop
    ctx.translate(mats.x[index], mats.y[index]);
    
    if (mats.refined[index] === 1) { // Remember, Uint8 uses 1 for true, 0 for false
        ctx.fillStyle = `rgba(8, 247, 208, ${mats.alpha[index]})`;
        mats.rotation[index] += 0.1;
    } else {
        ctx.fillStyle = `rgba(46, 191, 165, ${mats.alpha[index]})`;
    }
    
    ctx.rotate(mats.angle[index]);
    ctx.rotate(mats.rotation[index]);
...
function canvasDrawCollector(p) {
    ctx.save();
    ctx.translate(polarToCartesian(p.radius, p.angle).x, polarToCartesian(p.radius, p.angle).y);
    // ctx.rotate(p.angle);
...
// Save the game
function saveGame() {
    // 1. Create a lightweight copy of the planets array specifically for saving
    let planetsToSave = planets.map(p => {
        let cleanPlanet = { ...p };
        
        // 2. Instead of saving the massive buffers, extract ONLY the active rocks
        cleanPlanet.materials = { active: [] };
        
        for (let i = 0; i < p.materials.count; i++) {
            cleanPlanet.materials.active.push({
                radius: p.materials.radius[i],
                angle: p.materials.angle[i],
                rotation: p.materials.rotation[i],
                radiusChange: p.materials.radiusChange[i],
                timeInTractorBeam: p.materials.timeInTractorBeam[i],
                value: p.materials.value[i],
                alpha: p.materials.alpha[i],
                x: p.materials.x[i],
                y: p.materials.y[i],
                refined: p.materials.refined[i],
                isTargeted: p.materials.isTargeted[i]
            });
        }
        return cleanPlanet;
    });
    ...
    const gameState = {
        ...
        refineChainLevel, refineChainUpgradeCost, probes, probeParticles, showUpdateBox,
        
        planets: planetsToSave // Save our cleaned up version
    };
...
function loadGame() {
    ...
    // 1. Load the planets, and correctly reconstruct the Typed Arrays
    planets = state.planets.map(savedPlanet => {
        
        // FIX: Extract the saved rocks FIRST, before we touch anything!
        let savedActiveRocks = savedPlanet.materials ? (savedPlanet.materials.active || []) : [];
        
        // Now it is safe to attach the empty, maximum-size memory buffers
        savedPlanet.materials = getBaseDevices().materials;
        
        // Unpack the saved rocks back into the CPU buffers
        for (let i = 0; i < savedActiveRocks.length; i++) {
            let rock = savedActiveRocks[i];
            savedPlanet.materials.radius[i] = rock.radius;
            savedPlanet.materials.angle[i] = rock.angle;
            savedPlanet.materials.rotation[i] = rock.rotation;
            savedPlanet.materials.radiusChange[i] = rock.radiusChange;
            savedPlanet.materials.timeInTractorBeam[i] = rock.timeInTractorBeam;
            savedPlanet.materials.value[i] = rock.value;
            savedPlanet.materials.alpha[i] = rock.alpha;
            savedPlanet.materials.x[i] = rock.x;
            savedPlanet.materials.y[i] = rock.y;
            savedPlanet.materials.refined[i] = rock.refined ? 1 : 0; 
            savedPlanet.materials.isTargeted[i] = rock.isTargeted ? 1 : 0;
        }
        
        // Set the tracker to match how many rocks we just loaded in
        savedPlanet.materials.count = savedActiveRocks.length;
        return savedPlanet;
    });
